...
router.route("/getOwnerBuildingInfo/:email").get(async (req, res) => {
  ...
  try {
    await connection.query(
      `select flats.owner_email, flats.buildingID, flats.start_date, payment.paid, payment.due_date, payment.rent_amount from flats inner join payment on flats.flatID = payment.flatID and flats.buildingID = payment.buildingID where flats.tenant_email = "${tenant_email}"`,
